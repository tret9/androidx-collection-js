...
@Suppress("UNCHECKED_CAST") public fun intSetOf(): IntSet = EmptyIntSet

/** Returns a new read-only [IntSet] with only [element1] in it. */
@Suppress("UNCHECKED_CAST") public fun intSetOf(element1: Int): IntSet = mutableIntSetOf(element1)

/** Returns a new read-only [IntSet] with only [element1] and [element2] in it. */
@Suppress("UNCHECKED_CAST")
public fun intSetOf(element1: Int, element2: Int): IntSet = mutableIntSetOf(element1, element2)
 ...
 *
 * Though [IntSet] offers a read-only interface, it is always backed by a [MutableIntSet]. Read
 * operations alone are thread-safe. However, any mutations done through the backing [MutableIntSet]
 * while reading on another thread are not safe and the developer must protect the set from such
 * changes during read operations.
 *
 * @see [MutableIntSet]
 */
...
public class MutableIntSet(initialCapacity: Int = DefaultScatterCapacity) : IntSet() {
    ...
    /**
     * Trims this [MutableIntSet]'s storage so it is sized appropriately to hold the current
     * elements.
     *
